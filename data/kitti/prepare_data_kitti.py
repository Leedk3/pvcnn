...
def prepare_label(data_dir, output_dir):
    ...
    pc_path_dir = os.path.join(training_path, 'velodyne')
    label_path_dir = os.path.join(training_path, 'label_2')
    ...
    for pc_buf in pc_dir:
        ...
        pc_path = os.path.join(pc_path_dir, pc_ind + '.bin')
        label_path = os.path.join(label_path_dir, pc_ind + '.txt')


        if not os.path.exists(label_path):
            print(f'{label_path} already processed, skipping')
            continue

        print(pc_path, label_path)
        # xyz_object = np.loadtxt(pc_path)[:, :]  # (N,6)
        xyz_object = np.fromfile(pc_path, dtype=np.float32).reshape(-1, 4)
        print(xyz_object.shape)



    # for room in pc_path_dir:
    #     path_annotations = os.path.join(data_dir, area, room, 'Annotations')
    #     if not os.path.isdir(path_annotations):
    #         continue

        if os.path.exists(os.path.join(label_path)):
            print(f'{label_path} already processed, skipping')
            continue
...
def main():
    ...
    training_path = os.path.join(default_data_dir, 'training')
    if not os.path.isdir(training_path):
        print("No such directory.")
        return
    ...
    for pc_idx, pc_buf in enumerate(pc_dir):
        pc_ind = pc_buf.split('.', 1)[0]
        pc_path = os.path.join(pc_path_dir, pc_ind + '.bin')

        print(f'{datetime.now()}-Loading {pc_path}...')
        
        xyzi = np.fromfile(pc_path, dtype=np.float32).reshape(-1, 4)
        xyzi[:, 0:3] -= np.amin(xyzi, axis=0)[0:3]

        # print(xyzi.shape)

        label_path = os.path.join(label_path_dir, pc_ind + '.txt')

        print(f'{datetime.now()}-Loading {label_path}...')
        # labels = np.fromfile(label_path, dtype=np.int).flatten()
        # print(labels)
