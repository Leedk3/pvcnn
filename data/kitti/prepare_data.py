...
def extract_frustum_data(idx_filename, split, output_filename, viz=False,
    ...
    DATASET_DIR = os.path.dirname('/home/usrg/Dataset/LidarDetection/')
    print(DATASET_DIR)
    print(os.path.join(DATASET_DIR,'kitti'), split)
    ...
    for data_idx in data_idx_list:
        ...
        # img = dataset.get_image(data_idx)
        img_height, img_width, img_channel = (1382, 512, 3) #img.shape
        ...
        for obj_idx in range(len(objects)):
            ...
            for _ in range(augmentX):
                ...
                print("pc_in_box_fov:", pc_in_box_fov.shape)
                ...
                print("pc_in_box_fov2: ", pc_in_box_fov.shape)
                ...
                # print("box2d_list length: ", len(box2d_list))
                # print("box3d_list length: ", len(box3d_list))
                # print("input_list length: ", len(input_list))
                # print("label_list length: ", len(label_list))
                # print("type_list length: ", len(type_list))
                # print("heading_list length: ", len(heading_list))
                # print("box3d_size_list length: ", len(box3d_size_list))
                # print("frustum_angle_list length: ", len(frustum_angle_list))
    ...
    with open(output_filename,'wb') as fp:
        pickle.dump(id_list, fp) #0
        pickle.dump(box2d_list,fp) #1
        pickle.dump(box3d_list,fp) #2 
        pickle.dump(input_list, fp) #3 "lidar xyzi"
        pickle.dump(label_list, fp) #4 "label size: 1"
        pickle.dump(type_list, fp) #5 "string"
        pickle.dump(heading_list, fp) #6 "-3.14 ~ 3.14"
        pickle.dump(box3d_size_list, fp) #7 "3 x 1 : x, y, z size"
        pickle.dump(frustum_angle_list, fp) #8 ??
